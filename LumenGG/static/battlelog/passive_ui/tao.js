...
function setBusy(busy) {
    api.root.querySelectorAll("button").forEach((button) => {
        button.disabled = busy || !api.canControl;
    });
}
...
function syncHarmonyAfterCounterChange(yang, yin) {
    ...
    if (shouldActivateHarmony(yang, yin) && !harmonyStored) {
        return api.set("harmony", true, labels.harmony);
    }
    if (harmonyStored && !shouldKeepHarmony(yang, yin)) {
        return api.set("harmony_effect", "", labels.harmonyEffect)
            .then(() => api.set("harmony", false, labels.harmony));
    }
...
function setHarmonyEffect(effect) {
    ...
    const chain = harmonyStored
        ? Promise.resolve()
        : api.set("harmony", true, labels.harmony);
    chain
        .then(() => api.set("harmony_effect", effect, labels.harmonyEffect))
        .finally(() => setBusy(false));
}
...
function render() {
    ...
    yangValue.tabIndex = api.canControl && yang > 0 ? 0 : -1;
    yinValue.tabIndex = api.canControl && yin > 0 ? 0 : -1;
    yangValue.classList.toggle("is-disabled", !api.canControl || yang <= 0);
    yinValue.classList.toggle("is-disabled", !api.canControl || yin <= 0);
    ...
    harmonyRow.querySelectorAll("[data-tao-effect]").forEach((button) => {
        ...
        button.disabled = !api.canControl || !harmony;
    });
...
api.root.querySelectorAll("[data-tao-value]").forEach((valueNode) => {
    valueNode.addEventListener("click", () => {
        ...
        const current = counter === "yang" ? getYang() : getYin();
        if (!api.canControl || current <= 0) return;
